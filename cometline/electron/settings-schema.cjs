		...
		if (binding && typeof binding === 'object' && typeof binding.key === 'string') {
			...
			const fallback =
				def.id === 'toggleWebPanel'
					? normalizeToggleWebPanelBinding(void 0, def.defaultBinding)
					: normalizeSessionNavBinding(def.id, void 0, def.defaultBinding);
...
// src/lib/settings/schema.ts
var VALID_PROVIDER_METHODS = ['openai-compatible', 'openai', 'anthropic', 'opencode-go', 'codex'];
...
var providerSettingsSchema = external_exports.object({
	...
	app: external_exports.object({
		...
		miniWindowInactivityTimeoutMinutes: external_exports
			.number()
			.int()
			.min(1)
			.max(24 * 60),
	...
	cometmind: external_exports.object({
		...
		contextWindowLimit: external_exports.union([
			external_exports.literal(128e3),
			external_exports.literal(256e3)
		]),
		...
		mcp: external_exports.object({
			...
			servers: external_exports.array(
				external_exports.object({
					...
					env: external_exports
						.record(external_exports.string(), external_exports.string())
						.optional(),
					url: external_exports.string().optional(),
					headers: external_exports
						.record(external_exports.string(), external_exports.string())
						.optional(),
					oauth: external_exports
						.object({
